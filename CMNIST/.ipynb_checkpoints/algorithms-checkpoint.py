...
from lib.kde import get_grad_norm, Nonparametric
from lib.iro_utils import *
...
class ERM(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None, alpha=None):
        ...
        if alpha is None:
            ...
            t_alpha = torch.tile(alpha, (all_x.shape[0],1))
            loss = self.loss_fn(self.predict(all_x, t_alpha), all_y)
...
class Inftask(ERM):
    """
    Inf-task super quantile risk minimization.
    """
    def __init__(self, network, hparams, loss_fn):
        super().__init__(network, hparams, loss_fn)
        self.aggregator = aggregation_function(name='cvar')
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.grad_ratio = None

    def compute_cvar_h(self, alpha, minibatches):
        risks = []
        for x, y in minibatches:
            t_alpha = torch.tile(alpha,(x.shape[0],1))
            risks.append(self.loss_fn(self.network(x, t_alpha), y).reshape(1))
    ...
    def update(self, minibatches, unlabeled=None):
        # ERM pretraining
        if self.update_count < self.hparams["erm_pretrain_iters"]:
            return super().update(minibatches, unlabeled, alpha=torch.tensor(0.0).to(self.device))
...
class IRO(ERM):
    """
    IRO super quantile risk minimization.
    """
    def __init__(self, network, hparams, loss_fn):
        super(IRO, self).__init__(network, hparams, loss_fn)
        self.pareto_dist = Pareto_distribution(self.loss_fn)
        self.aggregator = aggregation_function(name='cvar')
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.grad_ratio = None
...
class IID(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # QRM objective
        env_risks = torch.cat([self.loss_fn(self.network(x), y).reshape(1) for x, y in minibatches])
        loss = self.aggregator.aggregate(env_risks, self.hparams["alpha"])
